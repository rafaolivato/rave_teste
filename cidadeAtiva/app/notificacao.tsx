...
export default function Notificacao(): JSX.Element {
  ...
  const [infosAdicionais, setInfosAdicionais] = useState<InfosAdicionais>({
    referencia: "",
    observacoes: "",
    foto: null,
  });
  ...
  const pegarLocalizacao = async (): Promise<void> => {
    let { status } = await Location.requestForegroundPermissionsAsync();
    if (status !== "granted") {
      Alert.alert("Permissão de localização negada!", "O app precisa de permissão para acessar sua localização.");
      return;
    }
    ...
    if (enderecoCompleto) {
      setEndereco(enderecoCompleto.street || "");
      setNumero(enderecoCompleto.name || "");
      setBairro(enderecoCompleto.subregion || "");
    }
  };

  // Função para selecionar imagem
  const escolherImagem = async (): Promise<void> => {
    ...
    <ScrollView contentContainerStyle={styles.container}>
            ...
            <TouchableOpacity
              onPress={() => {
                if (problemas.length > 0) {
                  ...
                  Alert.alert("Selecione um problema antes de continuar!");
                }
              }}
      ...
      {step === 3 && (
        <View style={styles.stepContainer}>
          ...
          <TextInput placeholder="Nome" style={styles.input} value={dados.nome} onChangeText={(v: string) => setDados({ ...dados, nome: v })} />
          <TextInput placeholder="Sobrenome" style={styles.input} value={dados.sobrenome} onChangeText={(v: string) => setDados({ ...dados, sobrenome: v })} />
          <TextInput placeholder="CPF" style={styles.input} value={dados.cpf} onChangeText={(v: string) => setDados({ ...dados, cpf: v })} />
          <TextInput placeholder="Telefone" style={styles.input} value={dados.telefone} onChangeText={(v: string) => setDados({ ...dados, telefone: v })} />
          <TextInput placeholder="Email" style={styles.input} value={dados.email} onChangeText={(v: string) => setDados({ ...dados, email: v })} />
      ...
      {step === 4 && (
        <View style={styles.stepContainer}>
          ...
          <TextInput
            ...
            onChangeText={(v: string) => setInfosAdicionais({ ...infosAdicionais, referencia: v })}
          />
          <TextInput
            placeholder="Observações"
            style={styles.input}
            value={infosAdicionais.observacoes}
            onChangeText={(v: string) => setInfosAdicionais({ ...infosAdicionais, observacoes: v })}
          />
      ...
      {step === 6 && (
        <View style={styles.stepContainer}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate("index" as never)} style={styles.buttonPrimary}>
